...
public class WalWriter implements TableWriterAPI {
    ...
    private final ObjList<NonGCCharSequenceIntHashMap> symbolMaps = new ObjList<>();
    ...
    private void configureEmptySymbol(int columnWriterIndex) {
        ...
        symbolMaps.extendAndSet(columnWriterIndex, new NonGCCharSequenceIntHashMap(8, 0.5, SymbolTable.VALUE_NOT_FOUND));
    ...
    private void resetSymbolMaps() {
        ...
        for (int i = 0; i < numOfColumns; i++) {
            final NonGCCharSequenceIntHashMap symbolMap = symbolMaps.getQuick(i);
            if (symbolMap != null) {
                symbolMap.clear();
            }
    ...
    private static class ConversionSymbolMapWriter implements SymbolMapWriterLite {
        ...
        private NonGCCharSequenceIntHashMap symbolHashMap;
        ...
        private int putSym0(int columnIndex, CharSequence utf16Value, SymbolMapReader symbolMapReader) {
            int key;
            if (utf16Value != null) {
                final NonGCCharSequenceIntHashMap utf16Map = symbolHashMap;
    ...
    private static class ConversionSymbolTable implements SymbolTable {
        ...
        public CharSequence valueOf(int key) {
            ...
            if (key < symbolCountWatermark) {
                ...
                return symbolHashMap.getFromIndex(keyIndex);
            }
        }
        ...
        void of(WalWriter writer, int columnIndex) {
            ...
            if (remapSize > 0) {
                ...
                for (int i = 0, n = symbolHashMap.size(); i < n; i++) {
                    CharSequence symbolValue = symbolHashMap.getFromIndex(i);
    ...
    private class RowImpl implements TableWriter.Row {
        ...
        private int putSym0(int columnIndex, CharSequence utf16Value, SymbolMapReader symbolMapReader) {
            int key;
            if (utf16Value != null) {
                final NonGCCharSequenceIntHashMap utf16Map = symbolMaps.getQuick(columnIndex);
                final int index = utf16Map.keyIndex(utf16Value);
                if (index > -1) {
                    key = symbolMapReader.keyOf(utf16Value);
                    if (key == SymbolTable.VALUE_NOT_FOUND) {
